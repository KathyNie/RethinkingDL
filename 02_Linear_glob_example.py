...
def global_quantile_regress(X_train,Y_train,pred_steps):
    quantiles = [0.5, 0.025, 0.975]
    models = {step: {} for step in range(pred_steps)}
    coefficients = {step: {} for step in range(pred_steps)}
    for step in range(pred_steps):
...
if __name__=="__main__":
    ...
    # Apply standardization per basin using the training set for glob linear (0:144)->(2003-2015)
    ...
    # Train the Linear_glob model
    OL_X_train_global_list = []
    OL_Y_train_global_list = []
    for i in range(515):
    
        _,_,X_train,Y_train,_,_=single_basin_data_prep(df_final_standardized_linearGlob,i,"OL",lag_num)
        OL_X_train_global_list.append(X_train)
        OL_Y_train_global_list.append(Y_train)

    OL_X_train_global_merged = pd.concat(OL_X_train_global_list, ignore_index=True)
    OL_Y_train_global_merged = pd.concat(OL_Y_train_global_list, ignore_index=True)

    OL_global_models,OL_global_coefficients = global_quantile_regress(OL_X_train_global_merged,OL_Y_train_global_merged,1)

    # Perform predictions
    OL_global_group_predictions = []
    OL_group_Y = []
    pred_steps = 1
    for i in range(515):
        X,Y,X_train,Y_train,train_col_index,time = single_basin_data_prep(df_final_standardized_linearGlob,i,"OL",lag_num)
        predictions_original_scale = {step:{q: scalers_linearGlob[basin_ids[i]]['OL_TWS'].inverse_transform(model.predict(X).reshape(-1,1)).flatten() for q, model in OL_global_models[step].items()} for step in range(pred_steps)}
        Y_original_scale = pd.DataFrame(scalers_linearGlob[basin_ids[i]]['OL_TWS'].inverse_transform(Y),columns=Y.columns, index=Y.index)    
        OL_global_group_predictions.append(predictions_original_scale)
        OL_group_Y.append(Y_original_scale)

    # Evaluate the predictions for the test period and save the results for analyses
    OL_global_evaluation_metrics = []

    for i in range(515):
        metric = combined_metrics_for_evaluation(OL_group_Y[i]["OL_TWS_t+0"][train_col_index+1:].values,OL_global_group_predictions[i][0][0.5][train_col_index+1:])
        OL_global_evaluation_metrics.append(metric)
        
    OL_global_evaluation_metrics=np.array(OL_global_evaluation_metrics)
    np.save('/path_to_output/np_save/OL_global_linear_quantile_regressor_metrics.npy', OL_global_evaluation_metrics)


    #Save the model to a file
    with open('/path_to_models/OL_global_quantile_regressors.pkl', 'wb') as file:
        pickle.dump(OL_global_models, file)
